 ...
 * 		http://update.wlanthermo.de/getSpiffs.php?device=nano&serial=xxxxxx&version=v0.0.0
 * ----------------------------------------------------------------------------------------------------------------------------------------
 */	
 
//-----------------------------------------------------------------------------
// include Logging libary 
$logfile = '_getSpiffs.log'; // global var for logger class filename
...
if (isset($_GET['device']) AND !empty($_GET['device']) AND isset($_GET['serial']) AND !empty($_GET['serial']) AND isset($_GET['version']) AND !empty($_GET['version'])){
	...
	// load Spiffs from database
	try {
		$sql = "SELECT spiffs_bin FROM `sw_versions` WHERE software_version=:software_version AND device=:device";
		...
		if(!empty($spiffs)){
			...
			header('Content-disposition: attachment; filename="spiffs_'.$_GET['version'].'.bin"');
