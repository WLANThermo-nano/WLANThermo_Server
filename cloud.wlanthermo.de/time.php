...
/* @author Florian Riedl 856d7a5cd57a0fd4
 */	
...
if (isset($_GET['api_token']) AND !empty($_GET['api_token'])){
	...
	}else if(isset($_GET['chartTime'])){
		$test = getTime($dbh,$_GET['api_token'],$history_time);
...
function getTime($dbh,$api_token,$api_time){
	try {
		echo "Zeitausgabe";
		echo '<br>';
		$sql = "SELECT data FROM `cloud` WHERE api_token= :api_token AND `time` > TIMESTAMP(DATE_SUB(NOW(), INTERVAL :history_time hour)) order by `id` asc";
			...
			foreach($statement as $key => $daten) {
				...
				echo '<tr>';
					if($key == 0){
						foreach ( $obj['channel'] as $key => $value )
						{
							$first_temp[$key] = $obj['channel'][$key]['temp'];
							$arr['channel'][$key]['temp'] = $value['temp'];
						}
						$first_ts = $obj['system']['time'];
					}
					if ($obj === null && json_last_error() !== JSON_ERROR_NONE) {
					}else{
						if($obj['channel'][0]['temp'] != 999){
							echo '<table>';
								echo '<thead>';
									echo '<tr>';
										echo '<th>Kanal</th>';
										echo '<th>Zeit</th>';
										echo '<th>Temp-IST</th>';
										echo '<th>Temp-SOLL</th>';
										echo '<th>Temp-DIFF</th>';
										echo '<th>Time-DIFF (s)</th>';
										echo '<th>Geschätzte Zeit (s)</th>';
									echo '</tr>';
								echo '</thead>';
								echo '<tbody>';
									foreach ( $obj['channel'] as $key => $value )
									{
										echo '<tr>';
											echo '<td align="left">'.$obj['channel'][$key]['number'].'</td>';
											echo '<td align="right">'.date('m/d/Y H:i:s', $obj['system']['time']).'</td>';
											echo '<td align="right">'.$obj['channel'][$key]['temp'].'</td>';
											echo '<td align="right">'.$obj['channel'][$key]['max'].'</td>';
											echo '<td align="right">'.($obj['channel'][$key]['max'] - $obj['channel'][$key]['temp']).'</td>';
											echo '<td align="right">'.($obj['system']['time'] - $first_ts).'</td>';
											echo '<td align="right">'.sprintf('%.2f',($obj['channel'][$key]['max'] - $obj['channel'][$key]['temp']) * ($obj['system']['time'] - $first_ts) / ($obj['channel'][$key]['temp'] - $first_temp[$key])).'</td>';
										echo '</tr>';
									}
								echo '<tbody>';
							echo '</table>';	
